...
class SendModals {
    ...
    static function renderError() {
        ?>
        <div id="error-modal" class="modal-bg hidden">
            <div class="modal flex flex-col bg-secondary space-y-3">
                <div class="flex justify-center">
                    <p class="w-10/12">
                        <?= Label::get('platform.step5.error') ?>
                    </p>
                </div>
                <button class="btn-primary mx-auto" type="button">OK</button>
            </div>
            <script>
              $('#error-modal button').onClick(() => $('#error-modal').addClass('hidden'));
            </script>
        </div>
        <?php
    }
